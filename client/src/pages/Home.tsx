...
const AethericDashboard = () => {
  ...
  const featureCards = [
    { title: '🍕 Order Food', description: 'Search restaurants & order meals' },
    { title: '🚕 Book Rides', description: 'Get cabs, autos & bikes instantly' },
    { title: '👷 Hire Workers', description: 'Find plumbers, electricians & more' }
  ];
    ...
    <div className="flex h-screen w-full bg-[#0a0a0a] text-white font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 h-full bg-[#0d0d0d] border-r border-[#1a1a1a] flex-col flex-shrink-0">
        ...
        <div className="px-3 mb-4">
          <button className="w-full flex items-center gap-3 px-3 py-3 rounded-xl bg-gradient-to-r from-indigo-600/20 to-purple-600/20 border border-indigo-500/30 text-sm font-medium text-indigo-300 hover:from-indigo-600/30 hover:to-purple-600/30 transition-all">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 pb-8 overflow-y-auto">
          {messages.length === 0 ? (
            ...
            <div className="w-full max-w-3xl flex-1 overflow-y-auto mb-4 space-y-4 pt-4 flex flex-col">
              {messages.map((m, i) => (
          ...
          {messages.length === 0 && (
            <div className="w-full max-w-3xl grid grid-cols-1 md:grid-cols-3 gap-4">
              {featureCards.map((card, i) => (
                <div key={i} className="bg-[#111111] border border-[#2a2a2a] rounded-xl p-4 hover:border-[#3a3a3a] transition-all cursor-pointer hover:scale-[1.02]">
